...
function workerBrief(b) {
  ...
  parts.push(
    ...
    + '- Process safety: when a test/script spawns a process, capture its pid at launch\n'
    + '  (`cmd & PID=$!`) and only ever signal `$PID`. NEVER `pgrep`/`pkill`/`kill` by name or\n'
    + '  pattern — your own agent process command line contains this whole brief, so a broad match\n'
    + '  can freeze or kill YOU.\n'

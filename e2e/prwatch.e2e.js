...
(async () => {
  ...
  try {
    ...
    await stepCase('server boots; lieutenant + direct-PR project registered (real clone from GitHub)', async () => {
      ...
      const r = await runCli(['project', 'add', repoUrl, '--name', 'scratch', '--workspace', ws, '--port', String(port)]);
    ...
    await stepCase('card start: REAL worker spawned for the direct-PR card', async () => {
      ...
      const created = await api('POST', '/api/cards', {
        id: CARD, title: 'PR watch e2e ' + RUN, owner: 'ada', playbook: 'default',
        attributes: { repo: 'scratch' }, body: cardBody,
      });

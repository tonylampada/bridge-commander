...
// page-in PAGE records at a time plus a "shown X of Y" footer. Clicking a row
...
export function renderArchive() {
  ...
  const head = '<tr><th>card</th><th>outcome</th><th>owner</th><th class="hide-m">labels</th>' +
    '<th class="hide-m">prs</th><th>archived</th><th class="hide-m">created</th></tr>';
  ...
  const html = '<div class="tv-scroll"><table class="tv"><thead>' + head + '</thead><tbody>' +
    (list.length ? list.map(rowHtml).join('')
      : '<tr><td colspan="7" class="tv-empty">' + (st.loading ? 'reading the archive…' : 'nothing archived matches') + '</td></tr>') +
...
function wire() {
  ...
  for (const tr of archEl.querySelectorAll('tbody tr[data-id]')) {
    tr.onclick = (e) => {

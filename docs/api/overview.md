...
| Operation | Signature | Who | When |
|---|---|---|---|
| `card.create` | `lieutenant, title, type, attrs → card` | 🤠 · ⚓ (proactive) | an idea/task is worth tracking; born in Backlog |
...
| Operation | Signature | Who | When |
|---|---|---|---|
| `chat.say` | `target: lieutenant-main \| card, text` | 🤠 ↔ ⚓ | any time; captain-side is write-ahead: queue first, then `harness.send` wake |

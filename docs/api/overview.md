...
| card leaves Working (`card.move`) | the worker's worktree is released — the handoff is the end of the work (`keep_worktree: true`, a worker that never reported done, and a worktree still holding work are the exceptions); a playbook's `teardown` runs in the worktree immediately before the release, best effort; a level-2 event names the path, and names the reason when the release is refused. A released worktree drops the card's `worktree` attribute — no reader is left pointed at a directory that is gone |
| card archived (any reason) | any worker session still bound to the card is killed (an archived card has neither Working nor worker); the worktree is released after the `card-archived` hooks — always, `keep_worktree` included, since nothing is left to rework. Already released at the handoff = a no-op (and the playbook's `teardown`, if any, ran there) |
...
the handoff, immediately before the worktree release, cwd = the worktree, same `BC_*` env
(`BC_EVENT=teardown`). `keep_worktree: true` runs neither it nor the release; a worktree
already released runs neither again.
...
one that failed. Best effort: a non-zero exit or a timeout (5 min, `BC_TEARDOWN_TIMEOUT_MS`
overrides, then the whole process group is killed) lands its event, queues to the owner, and
the release goes ahead exactly as if no teardown had been configured. Nothing is lost by
carrying on — a container still holding the checkout makes the release refuse on its own, and
say why.

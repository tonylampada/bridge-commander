...
| `lieutenant.patch` | `color?, avatar?, voice?, name?, charter?, ref? → lieutenant` | 🤠 (⋯ → settings) · ⚙️ (ref re-registration on init idempotency) | cosmetics + voice + charter; `name` changes the display only — `id` and the derived session name stay immutable; `avatar: null` clears back to the colored-dot fallback; `voice: ""`/`null` clears back to the board's voice |

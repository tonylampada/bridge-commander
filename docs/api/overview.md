...
`keep_worktree: true` card keeps its worker through the move, and a send reopens it in place. Lieutenant lane sits above the columns; each lieutenant has a color,
...
| `card.start` | `card, {brief?, resume?, harness?, model?, effort?} → worker` | ⚓ (own judgment, or executing a captain drag-order) | ready to work: ONE atomic op — spawn worker window + worktree, bind to card, card → Working. The card's `playbook` is resolved and rendered HERE and only here — title, body, thread and attributes as they stand at this instant, since all four keep changing until then; a card with no playbook is REFUSED, with the available playbooks named (no fallback, no default picked on the card's behalf). Harness/model resolve: explicit arg → the playbook's frontmatter → config default (`harness` only — there is no configured default model, the harness picks its own); a playbook's `requires` is checked BEFORE anything is provisioned, so a card missing an attribute is refused with the attribute named rather than spawning a worker to discover it. The rendered brief is auto-attached as a card artifact (label `brief`, idempotent across resumes); `--resume` reincarnates the recorded worker instead (refuses a brief — steer with `worker.send`; refuses too once the card has been handed off, because the handoff killed that worker and dropped its record — there is nothing left to reincarnate, and a fresh start is the way back. The ONE carve-out is the handoff whose release REFUSED: that record is deliberately kept, and resuming it is deliberately allowed — the worktree it names may hold the only copy of the work, and a resume in place is the way back into it). Every start reads the playbook — there is no second way for a card to begin. Whether a branch is cut is the playbook's call (`branch:` in its frontmatter), falling back to the card type when the block does not say. Implementation cards get branch `bc/<card-id>` — `bc/MON-14`, the id and nothing else, deliberately: the captain reads branch names and wants them aligned with the card, and the PR carries a title for anyone who wants prose. Investigations get NO branch — their deliverable is the report. A card with NO worker record that still carries a `worktree` attribute — the pointer a handoff leaves when its release never landed, and what `card.restore` brings back for a card archived after a refused release — has its start release against that pointer on the same terms: 409 when the checkout still holds work, and the pointer cleared when the ground goes. A pointer is not ownership, though — a pooled lease named by a stale snapshot may already belong to another card's LIVE worker, and the start refuses (409, naming that card) rather than releasing ground out from under it. Without it the reprovision dead-ends on a path that already exists with nothing left to release against. A restart over a finished, dead worker releases that worker's worktree first, running the PREVIOUS run's `teardown` (60s, awaited) in it immediately before — the restart is the moment that checkout is actually destroyed, and it is the one release point a `keep_worktree` playbook ever reaches; a release that refuses still 409s the start. The old record is dropped on the same VERIFIED terms as the handoff's: a pane that cannot be shown to be gone (the kill throws, or its harness is unreachable) 409s the start naming the session to end by hand, rather than spawning a second worker over a live one nothing points at. `plan` cards never start |
| `card.move` | `card, column` | 🤠 drag = **order** · ⚓ only → Your review (the handoff) · ⚙️ only on objective facts (start, merge) | see side effects for drag semantics. A move OUT of Working ends the work, so it ends the worker: the window is KILLED and its record dropped, and the worktree released. The lieutenant has read the diff by then, and a card sitting in review otherwise pinned an idle agent process and a full checkout until someone archived it. Rework after a handoff is a fresh start, by the rule above. Neither the kill nor the release happens for a playbook with `keep_worktree: true` (a card reworked in place) or a worker that never reported done (that checkout and that conversation may be the only copy); a worktree still holding work is kept on its own account, the kill goes ahead regardless |
...
| worker stop | — | ⚙️ turn-end | a worker turn-end IS the stop signal: card still Working and no `done` → immediate `worker-stopped` QueueItem to the owner + level-2 event, EVERY such turn-end (a worker re-sent after a stop that ends its turn again with no signal has stopped again, and an owner who heard only the first stop is the silence of CMD-26 — `test/workers.test.js`). A turn-end also resets the stall ladder. After `done`, turn-ends only update counters |
...
| worker stall | — | ⚙️ supervision loop | a worker alive but silent too long (no signal/turn-end) → `worker-stalled` event + QueueItem to the owner, RINGING AGAIN once per `BC_WORKER_STALE_SECS` (default 1800) of continued silence: the first hit is level 2 (the lieutenant's), the second and every later hit level 1 (the captain's), numbered and quoting the worker's last word (whichever is newer of the turn-end hook's `text` and the last signal). Any real activity — signal, turn-end, resume, send-reopen, pause, leaving Working — resets the ladder, so the next stall starts quiet again. Events carry the level explicitly; the kinds map is not consulted |
...
| Verb | Signature | Called by | Purpose |
|---|---|---|---|
| `harness.openPane` | `ref, {onFrame, intervalMs?, lines?, burstMs?, burstWindowMs?} → {close()}` | ⚙️ pane hub | stream the pane's rendered screen as change-detected frames (`intervalMs` defaults to ~1000, `lines` — scrollback depth — to ~200; `burstMs`/`burstWindowMs` are the paneInput burst, see `harness/README.md`) (strings, MAY carry ANSI SGR) — served to the captain over a dedicated per-target SSE (`GET /api/cards/:id/pane/stream`, `GET /api/lieutenants/:id/pane/stream`), ref-counted so N viewers share ONE feed and the last disconnect releases it; harness lacks the verb → `unsupported`, nothing to watch → `no-pane`, concurrent-pane cap → `busy` (all clean SSE events, never an HTTP error) |
...
| `harness.commands` | `ref? → [{name, description, args?}]` | ⚙️ | list the slash commands this session honors (drives the UI's command palette). `ref`, when given, SCOPES the answer to that session — claude's `/output-style` offers the styles installed in the session's own cwd on top of the user-level ones. `args` is OPTIONAL metadata (`[{value, description}]`): the values the command accepts as its SINGLE argument, so the composer can keep completing AFTER the command name (`ui/js/slash.js`); a harness that omits it behaves exactly as before — the picker closes on the space. Everything typed after the command name is ONE argument: a value may contain spaces and is never tokenized |
...
3. **Working ⇔ unfinished task, which SHOULD have a live worker.** The way into Working is `card.start`, spawning the worker atomically (or `worker.send` reopening a done-but-alive worker, which is legal only while the card has not left Working — the move out kills it — EXCEPT on a `keep_worktree: true` card, whose worker survives the handoff by design and is sent to in `Your review` to rework the card in place). A Working card may lose its worker only by accident (process died, machine rebooted) — the server flags it and queues the owner; a wound to heal — or by `worker.pause`, the ONE deliberate stop, marked so supervision never reads it as a wound.
...
| `worker.send` by the lieutenant | text typed into the card's live worker session (harness `send`, verified submission) + level-2 event; on a done-but-alive worker it REOPENS the turn (record reset, card → Working) — send = "more work for this worker" — which the handoff closes for good: it killed the worker and dropped its record, so a send after it is the loud no-worker error, pointing at a fresh start. The exception is the handoff's own: a `keep_worktree: true` card keeps worker and checkout through the move, so a send in `Your review` still reopens that same conversation — the rework-in-place flow the key exists for. `card.start --resume` refuses a brief and points here |
...
| worker alive but silent past the stall window | `worker-stalled` event + QueueItem to the owner, re-fired every window while the silence lasts — level 2 first, level 1 from the second hit on (see *worker stall* above) |
...
| server boot | one sweep of the worker registry: every record whose card is off the board, or on it but not in Working, has its window killed and its record dropped (the handoff's exceptions — `keep_worktree: true`, and a worker that never reported done — apply to a card still on the board and spare it entirely, plus one of the sweep's own that spares the RECORD only: a record whose worktree is still UNRELEASED has its window killed like any other, but the entry survives, because it is the last handle on the work standing there, which is exactly what `card.park` and a refused release each leave behind. A record with no worktree holds nothing and goes. Off the board there are no exceptions: no card will ever come back for it). A record the board has already failed to end, whose card is no longer on the board, is the sweep's one terminal path: it is dropped anyway on the next failure, with a level-2 event naming the session left running — keeping a record protects work somebody may come back for, and nobody is coming back for an archived card. A record whose card is still on the board is kept, and keeps refusing `card.start`. A worker outlives neither its card's Working state nor a restart of the board that forgot to notice |

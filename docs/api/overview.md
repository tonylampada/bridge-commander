...
| `harness.runCommand` | `ref, line → string` | ⚙️ | run one slash-command line in the session (pass-through or emulated per harness) and return the reply text. Unknown names throw — and so does a command whose argument is missing or unrecognised, BEFORE it does anything: claude's `/output-style` writes the style into the session's own `.claude/settings.local.json` and RESTARTS the session (kill + resume) to apply it, since the setting is read at process start, so a typo must not cost a restart. A command that restarts its session also refuses while that session is MID-TURN — a resume comes back on an idle composer, so an interrupted turn is lost, not continued |
...
| `chat.say` starting with `/` | routed to `harness.runCommand`, reply lands in-thread — NO QueueItem, no wake, no owed. On a card target the command addresses the WORKER session (unlike say, which always talks to the owner). A command that deliberately RESTARTS its session (claude's `/output-style`; the board's own `/reset`, which respawns a lieutenant outright) is marked for the duration of the call, so supervision reads neither the gap as a crash nor races the restart with a respawn of its own |

...
| card leaves Working (`card.move`) | the worker DIES — its window is killed and its registry record dropped — and its worktree is released; the handoff is the end of the work (`keep_worktree: true` and a worker that never reported done are the exceptions to BOTH; a worktree still holding work is kept on its own account, and the kill goes ahead anyway). The card keeps `session` and `resumeId` so the run stays readable afterwards. The kill is VERIFIED — the record is dropped only once the pane is provably gone; a pane that survives it keeps its record and lands a level-1 event on the card, never a silent leak. A REFUSED release keeps the record too (the window is dead either way): that entry is the last handle on the work still standing on that checkout, and archive is where it gets its next turn. A playbook's `teardown` runs in the worktree immediately before the release, best effort, 5 min; a level-2 event names the path, and names the reason when the release is refused. A released worktree drops the card's `worktree` attribute — no reader is left pointed at a directory that is gone |
...
| server boot | one sweep of the worker registry: every record whose card is off the board, or on it but not in Working, has its window killed and its record dropped (the handoff's exceptions apply to a card still on the board). A worker outlives neither its card's Working state nor a restart of the board that forgot to notice |

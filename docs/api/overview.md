...
| `card.start` | `card, {brief?, command?, resume?, harness?, model?, effort?} → worker` | ⚓ (own judgment, or executing a captain drag-order) | ready to work: ONE atomic op — spawn worker window + worktree, bind to card, card → Working. The card's `brief` template is resolved and rendered HERE and only here — title, body, thread and attributes as they stand at this instant, since all four keep changing until then; a card with no brief is REFUSED, with the available templates named (no fallback, no default picked on the card's behalf). Harness/model resolve: explicit arg → the template's frontmatter → config default; a template's `requires` is checked BEFORE anything is provisioned, so a card missing an attribute is refused with the attribute named rather than spawning a worker to discover it. The rendered brief is auto-attached as a card artifact (label `brief`, idempotent across resumes); `--resume` reincarnates the recorded worker instead (refuses a brief — steer with `worker.send`). `--command` starts the session on that command line instead of an agent with a brief (the `command` harness) — same worktree, branch, registry entry and supervision; the two are mutually exclusive and `worker.send` on such a worker fails loudly. On a brief-driven start, whether a branch is cut is the brief's call (`branch:` in its frontmatter), falling back to the card type when the block does not say; a `--command` start reads no template at all, so there the card type decides. Implementation cards get branch `bc/<card-id>` — `bc/MON-14`, the id and nothing else, deliberately: the captain reads branch names and wants them aligned with the card, and the PR carries a title for anyone who wants prose. Investigations get NO branch — their deliverable is the report. `plan` cards never start |
